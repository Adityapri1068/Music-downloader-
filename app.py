...
if url:
    with yt_dlp.YoutubeDL({'quiet': True}) as ydl:
        try:
            ...
            format_choice = st.radio("Select format", ['MP3 (Audio)', 'MP4 (Video)', 'Image only'])

            if st.button("Download"):
                if format_choice == 'MP3 (Audio)':
                    ydl_opts = {
                        'format': 'bestaudio/best',
                        'outtmpl': os.path.join(download_dir, f'{title}.mp3'),
                        'postprocessors': [{
                    ...
                    st.audio(os.path.join(download_dir, f'{title}.mp3'))

                elif format_choice == 'MP4 (Video)':
                    ydl_opts = {
